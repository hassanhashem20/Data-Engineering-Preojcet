...
SELECT * FROM Reservations WHERE Notified = 1 AND Status = 'active';
SELECT b.ISBN, b.StatusID, s.StatusName
FROM Book b
LEFT JOIN Book_Status s ON b.StatusID = s.StatusID
WHERE b.ISBN = '12212121';



UPDATE Book
SET StatusID = (SELECT StatusID FROM Book_Status WHERE StatusName = 'Borrowed' LIMIT 1)
WHERE ISBN IN (
    SELECT ISBN FROM Borrowings WHERE ReturnDate IS NULL
);
